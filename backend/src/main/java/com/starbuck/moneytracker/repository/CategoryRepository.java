...
public interface CategoryRepository extends JpaRepository<Category, Long> {
     ...
     * @return
     */
    @Query("SELECT COUNT(c) > 0 FROM Category c WHERE c.name = ?1 AND (c.user.id = ?2 OR c.user.id IS NULL) AND c.status = 0")
    boolean isUserHaveThisCategoryName(String name, long userId);
